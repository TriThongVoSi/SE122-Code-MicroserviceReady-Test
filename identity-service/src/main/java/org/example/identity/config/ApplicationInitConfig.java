...
public class ApplicationInitConfig {
    ...
    ApplicationRunner applicationRunner(
        ...
        return args -> {
            ...
            org.example.identity.entity.Role adminRole = ensureRoleExists(
                    PredefinedRole.ADMIN_ROLE,
                    "Admin",
                    "Administrator user with full access",
                    roleRepository);
            org.example.identity.entity.Role farmerRole = ensureRoleExists(
                    PredefinedRole.FARMER_ROLE,
                    "Farmer",
                    "Farmer user",
                    roleRepository);
            org.example.identity.entity.Role employeeRole = ensureRoleExists(
                    PredefinedRole.EMPLOYEE_ROLE,
                    "Employee",
                    "Employee user",
                    roleRepository);
            org.example.identity.entity.Role buyerRole = ensureRoleExists(
    ...
    private org.example.identity.entity.Role ensureRoleExists(String code, String name, String description, RoleRepository roleRepository) {
        return roleRepository.findByCode(code)
                .orElseGet(() -> {
                    log.info("Creating default role with code: {}", code);
                    org.example.identity.entity.Role role = org.example.identity.entity.Role.builder()
                            .code(code)
                            .name(name)
    ...
    private void ensureUserExistsWithRole(String username, String email, String password,
            String fullName, String phone, org.example.identity.entity.Role role,
            UserRepository userRepository) {
        Optional<org.example.identity.entity.User> existingUserByEmail = userRepository.findByEmailIgnoreCaseWithRoles(email);
        Optional<org.example.identity.entity.User> existingUserByUsername = userRepository.findByUsernameWithRoles(username);

        if (existingUserByEmail.isPresent()) {
            org.example.identity.entity.User user = existingUserByEmail.get();
        ...
        if (existingUserByUsername.isPresent()) {
            org.example.identity.entity.User user = existingUserByUsername.get();
        ...
        var roles = new HashSet<org.example.identity.entity.Role>();
        roles.add(role);

        org.example.identity.entity.User newUser = org.example.identity.entity.User.builder()
                .username(username)
                .email(email)
                .fullName(fullName)
                .phone(phone)
