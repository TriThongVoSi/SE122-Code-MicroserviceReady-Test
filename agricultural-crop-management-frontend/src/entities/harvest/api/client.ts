...
export const harvestApi = {
    ...
    getStockContext: async (
        seasonId: number,
        params: { warehouseId: number; productName: string; lotCode: string }
    ): Promise<HarvestStockContext> => {
        const response = await httpClient.get(`/api/v1/seasons/${seasonId}/harvests/stock-context`, { params });
        return parseApiResponse(response.data, HarvestStockContextSchema);
    },
