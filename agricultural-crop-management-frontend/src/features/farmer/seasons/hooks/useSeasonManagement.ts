...
import { laborApi } from '@/entities/labor';
...
export function useSeasonManagement() {
  ...
  const confirmStartSeason = useCallback((data: {
      ...
      { id: numId, data: payload },
      {
        onSuccess: async () => {
          ...
          if (data.employeeUserIds && data.employeeUserIds.length > 0) {
            try {
              await laborApi.bulkAssignSeasonEmployees(numId, {
                employeeUserIds: data.employeeUserIds,
              });
              toast.success(`Assigned ${data.employeeUserIds.length} employees to season`);
            } catch (error: any) {
              toast.error('Season started, but employee assignment failed', {
