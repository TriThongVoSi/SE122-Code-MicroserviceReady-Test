...
const SoilTestFormSchema = z.object({
  ...
  mineralNKgPerHa: z
    .number({ required_error: 'Mineral N là bắt buộc' })
...
const SOURCE_OPTIONS: { value: SoilTestSourceType; label: string }[] = [
...
export function SeasonSoilTestsWorkspace() {
  ...
  const createMutation = useCreateSoilTest(seasonIdNumber, {
    onSuccess: () => {
      toast.success('Đã lưu soil test');
      form.reset({
        ...defaultValues(),
    ...
    onError: (error) => {
      toast.error(error.message || 'Không thể lưu soil test');
    },
  });
  ...
  const seasonWriteLockReason = isSeasonWriteLocked
    ? 'Mùa vụ đã khóa. Không thể ghi mới soil test.'
    : undefined;
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          {!plotId && !isSeasonLoading && (
            <Alert variant="destructive">
              ...
              <AlertDescription>Mùa vụ này chưa có plotId, chưa thể submit soil test.</AlertDescription>
            </Alert>
          )}
            ...
            <form onSubmit={onSubmit} className="space-y-4" data-testid="soil-test-form">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Mineral N (kg N/ha) *</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          step="0.0001"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Soil organic matter (%)</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          step="0.0001"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Nitrate (mg/kg)</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          step="0.0001"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Ammonium (mg/kg)</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          step="0.0001"
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          value={field.value ?? ''}
                          onChange={field.onChange}
                          placeholder="Ví dụ: soil-lab-report.pdf"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input value={field.value ?? ''} onChange={field.onChange} placeholder="LAB-SOIL-21" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        ...
                        value={field.value ?? ''}
                        onChange={field.onChange}
                        placeholder="Mô tả bổ sung về mẫu đất..."
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
      ...
      <Card className="border border-border rounded-2xl shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">Danh sách soil tests</CardTitle>
        </CardHeader>
        <CardContent>
          {isListLoading ? (
            <div className="text-sm text-muted-foreground flex items-center gap-2">
              <Loader2 className="w-4 h-4 animate-spin" />
              Đang tải danh sách soil tests...
            </div>
          ) : records.length === 0 ? (
            <p className="text-sm text-muted-foreground">Chưa có bản ghi soil test cho mùa vụ này.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    ...
                    <th className="text-left font-medium py-2 pr-3">Mineral N (kg/ha)</th>
                    <th className="text-left font-medium py-2 pr-3">SOM (%)</th>
