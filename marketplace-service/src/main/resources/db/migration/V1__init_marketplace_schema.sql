...
CREATE TABLE IF NOT EXISTS marketplace_order_audit_logs (
    ...
    note TEXT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_marketplace_order_audit_logs_order FOREIGN KEY (order_id) REFERENCES marketplace_orders(id) ON DELETE CASCADE,
    INDEX idx_marketplace_order_audit_logs_order_id (order_id)
);
